...
int main()
{
	fio;
	lg2[1]=0;
	for (i=2;i<=1000;i++) lg2[i]=lg2[i/2]+1;
	cin>>n>>m;
	...
	for (t=1;t<=q;t++)
	{
		...
		y1--;
		y2--;
		int l=0,r=min(x2-x1+1,y2-y1+1),mid;
		while(l<r)
		{
			mid=(l+r)/2+1;
	//		cout<<"getting max "<<x1<<' '<<y1<<' '<<x2-mid+1<<' '<<y2-mid+1<<endl;
	//		cout<<get_max(x1,y1,x2-mid+1,y2-mid+1)<<endl;
			if (get_max(x1,y1,x2-mid+1,y2-mid+1)>=mid) l=mid;
			else r=mid-1;
		}
		cout<<l<<endl;
	}
