...
int main()
{
	...
	for (j=0;j<=2*m;j++) for (i=1;i<=n;i++)
	{
		if ((i>cnt[i])or(i<j)or(cnt[i]<j)) u=0;
		else 
		{
			u=1;
			u*=fac[cnt[i]-j];
		...
		pre[i][j]=(pre[i-1][j]+u)%MOD;
	}
	for (i=0;i<m;i++) cin>>ban[i][0]>>ban[i][1];
	for (i=0;i<(1<<m);i++)
	{
		u=0;
		a=1;
